...
if args.show_B:
    mlab.points3d(pos[:,0], pos[:,1], pos[:,2], scale_mode='none', scale_factor=0.2,
                  color=(0.9, 0.6, 0))

# scaled value of sigma such that the norm of the vector (scaled_sigma, scaled_sigma,
# scaled_sigma) is 2*sigma, that is the diameter we wish for the sphere glyph
scaled_sigma = 2*sigma/np.sqrt(3)

# From the mayavi documentation, this is how to size and color spheres independently, here
# size is given by 2*sigma (diameter) and color by -dimer_species, so that the catalytic
# bead (species=1) is red and the non-catalytic one is blue (species=2) with the default
# colormap in mayavi
dimer_plot = mlab.quiver3d(dimer_all_pos[-1,:,0], dimer_all_pos[-1,:,1],
                           dimer_all_pos[-1,:,2], scaled_sigma, scaled_sigma, scaled_sigma,
                           scalars=-dimer_species, mode='sphere', scale_factor=1,
                           scale_mode='vector', resolution=20)
dimer_plot.glyph.color_mode = 'color_by_scalar'
dimer_plot.glyph.glyph_source.glyph_source.center = [0, 0, 0]

if args.unwrap:
    mlab.plot3d(dimer_all_pos[:,0,0], dimer_all_pos[:,0,1], dimer_all_pos[:,0,2],
                line_width=10, color=(0.8, 0.4, 0))
