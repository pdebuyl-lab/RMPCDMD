...
!! \param delta_u               reaction exothermicity
...
program single_body_thermal
  ...
  delta_u = PTread_d(config, 'delta_u', loc=params_group)
  ...
  do i = 0, N_loop+equilibration_loops
     ...
     if (sampling) then
        ...
        call so_timer%tac()
        e4 = n_solvent(1)*delta_u
        call thermo_data%append(hfile, temperature, e1+e2+e3+e4+e_wall, kin_e, e1+e2+e3+e4+e_wall+kin_e, v_com)
  ...
  e4 = n_solvent(1)*delta_u
  call thermo_data%append(hfile, temperature, e1+e2+e3+e4+e_wall, kin_e, e1+e2+e3+e4+e_wall+kin_e, v_com, add=.false., force=.true.)
  if (do_solvent_io) then
...
end program single_body_thermal
